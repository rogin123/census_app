...
library(tidyverse)
library(sf)
# census-app-ggplot2 - L13 ------------------------------------------------

# scratch work for function 

test_function <- function(var = "white", color = "darkgreen",
                          legend_title = "% White",
                          min = 0, max = 100, 
  ...
  legend_labels <- c(paste0(if_else(min ==0, "0%", " % or less")),
                   ...
                   paste0(min + 2 * inc, "%"),
                   paste0(min + 3 * inc, "%"),
                   paste0(max, if_else(max == 100, "100%", "% or more")))
...
test_function(min = 0, max = 80, state_selection = "florida")

test_function(var = "black", min = 20, max = 80)
